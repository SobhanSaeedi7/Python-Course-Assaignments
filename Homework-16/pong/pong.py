...
class Game(arcade.Window):
    def __init__(self):
        ...
        self.player_2 = Rocket(self.width - 40, self.height//2, arcade.color.GREEN_YELLOW)
        self.ball = Ball(self)
    ...
    def on_draw(self):
        ...
        self.player_2.draw()
        self.ball.draw()
        arcade.draw_text(f"Player Score: {self.player_1.score}", 30, 25 , arcade.color.RED, 20, bold= True)
        arcade.draw_text(f"Player Score: {self.player_2.score}", self.width-230, 25 , arcade.color.GREEN_YELLOW, 20, bold= True)
    ...
    def on_update(self, delta_time):
        ...
        if arcade.check_for_collision_with_list(self.ball, self.players) and self.ball.hit == True:
